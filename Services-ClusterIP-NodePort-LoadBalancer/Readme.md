...
- port: 80              # Service port (cluster-internal)
  ...
  How ClusterIP Works
  IP Assignment: Kubernetes assigns a virtual IP from cluster's internal range (10.0.0.0/24 by default)
  Internal Routing: Traffic only routable within cluster via kube-proxy
  Load Balancing: Distributes requests across matching pods using iptables/ipvs rules
  DNS Resolution: Automatic DNS record creation (e.g., my-app-svc.default.svc.cluster.local)
  Service Discovery: Pods access service by name, resolved by cluster DNS
...
- port: 80             # Service port (load balancer port)
  ...
  Cloud Integration: Interacts with cloud provider's load balancer service
  External IP: Automatically provisions external IP/load balancer
  Traffic Distribution: Cloud LB distributes traffic across healthy pods
  Health Checks: Automatic health monitoring of pods
...
13. Cluster Network Plugin (Flannel, Calico, WeaveNet, etc.)
...
kubectl run trace-pod --image=nicolaka/netshoot --rm -it -- traceroute <service-ip></service>

# Check kube-proxy logs

kubectl logs -n kube-system -l k8s-app=kube-proxy -f
Why This Architecture Matters
Scalability: Nodes don't need to know about every pod
Resilience: Pod failures don't break service routing
Load Distribution: Traffic automatically spreads across healthy pods
Simplicity: Applications see stable service IP regardless of pod location
Multi-Cloud: Same architecture works on any cloud or on-premises
...
curl http://<external-ip></external>
...
Common Patterns
Internal Services: Use ClusterIP for microservice communication
External APIs: Use LoadBalancer for public APIs
Development: Use NodePort for quick testing
Hybrid: Combine multiple service types in same application
...
kubectl describe svc <service-name></service>

🐛 Troubleshooting
Common Issues
Image Pull Errors

kubectl describe pod <pod-name></pod>
...
kubectl exec -it <pod-name></pod> -- nslookup <service-name></service>
